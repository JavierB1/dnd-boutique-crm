...
// ─── CONFIGURACIÓN N8N ────────────────────────────────────────
const N8N_WEBHOOK = "https://n8n-production-9ae2.up.railway.app/webhook/497950a0-e581-4736-b3df-408cc14d168f/webhook";
const WHATSAPP_TOKEN = ""; // Pon tu token aquí para enviar mensajes
const PHONE_NUMBER_ID = "1005186782689308";

// ─── DATOS INICIALES ──────────────────────────────────────────
const initialCombos = [
  { id: 1, nombre: "Combo Dúo", descripcion: "2 perfumes a elegir", precio: 25, stock: 15, activo: true },
  { id: 2, nombre: "Combo Trío", descripcion: "3 perfumes a elegir", precio: 35, stock: 10, activo: true },
  { id: 3, nombre: "Combo Premium", descripcion: "2 perfumes línea premium", precio: 45, stock: 8, activo: true },
  { id: 4, nombre: "Combo Familiar", descripcion: "5 perfumes variados", precio: 55, stock: 5, activo: false },
];

const initialClients = [
  { id: "50372436365", nombre: "Javier B", telefono: "+50372436365", status: "Nuevo", totalGastado: 0, notas: "" },
];
...
export default function CRM() {
  ...
  const [stats, setStats] = useState({ mensajeshoy: 0, bot: 0, humano: 0, ventas: 0 });
  ...
  useEffect(() => {
    const poll = async () => {
      try {
        const res = await fetch(`${N8N_WEBHOOK}?crm=1`, {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        });
        if (res.ok) {
          const data = await res.json();
          if (data?.messages) procesarMensajesEntrantes(data.messages);
        }
      } catch (e) {
        // silencioso
      }
  ...
  const procesarMensajesEntrantes = (mensajes) => {
    mensajes.forEach(msg => {
      const telefono = msg.from || msg.wa_id;
      const nombre = msg.name || telefono;
      ...
      setClients(prev => {
        if (prev.find(c => c.id === telefono)) return prev;
        return [...prev, { id: telefono, nombre, telefono: `+${telefono}`, status: "Nuevo", totalGastado: 0, notas: "" }];
      });

      // Agregar mensaje a conversación
      setConversations(prev => {
        const conv = prev[telefono] || { mensajes: [], sinLeer: 0, botActivo: true };
        return {
          ...prev,
          [telefono]: {
            ...conv,
            mensajes: [...conv.mensajes, { from: "client", texto, tiempo, nombre }],
            sinLeer: conv.sinLeer + 1,
            ultimoMsg: texto,
            ultimoTiempo: tiempo,
          }
        };
      ...
      // Notificación
      setNotification({ nombre, texto });
      setTimeout(() => setNotification(null), 4000);

      setStats(prev => ({ ...prev, mensajeshoy: prev.mensajeshoy + 1, bot: prev.bot + 1 }));
    });
  };

  // Simular mensaje entrante para demo
  const simularMensaje = () => {
    const demos = [
      { telefono: "50372436365", nombre: "Javier B", texto: "Hola, me interesan sus perfumes!" },
      { telefono: "50371234567", nombre: "María López", texto: "Cuánto cuesta el combo dúo?" },
      { telefono: "50379876543", nombre: "Carlos R", texto: "Hacen envíos a Santa Ana?" },
    ];
    const demo = demos[Math.floor(Math.random() * demos.length)];
    const tiempo = new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });

    setClients(prev => {
      if (prev.find(c => c.id === demo.telefono)) return prev;
      return [...prev, { id: demo.telefono, nombre: demo.nombre, telefono: `+${demo.telefono}`, status: "Nuevo", totalGastado: 0, notas: "" }];
    });

    setConversations(prev => {
      const conv = prev[demo.telefono] || { mensajes: [], sinLeer: 0, botActivo: true };
      return {
        ...prev,
        [demo.telefono]: {
          ...conv,
          mensajes: [...conv.mensajes, { from: "client", texto: demo.texto, tiempo, nombre: demo.nombre }],
          sinLeer: conv.sinLeer + 1,
          ultimoMsg: demo.texto,
          ultimoTiempo: tiempo,
        }
      };
    });

    setNotification({ nombre: demo.nombre, texto: demo.texto });
    setTimeout(() => setNotification(null), 4000);
    setStats(prev => ({ ...prev, mensajeshoy: prev.mensajeshoy + 1, bot: prev.bot + 1 }));
  };
  ...
  const enviarMensaje = async () => {
    ...
    setConversations(prev => ({
      ...
      [selectedPhone]: {
        ...prev[selectedPhone],
        mensajes: [...(prev[selectedPhone]?.mensajes || []), { from: "user", texto: newMsg, tiempo }],
        ultimoMsg: newMsg,
        ultimoTiempo: tiempo,
      }
    }));
  ...
  const toggleBot = (telefono) => {
    setConversations(prev => ({
      ...prev,
      [telefono]: { ...prev[telefono], botActivo: !prev[telefono]?.botActivo }
    }));
  };

  const marcarLeido = (telefono) => {
    setConversations(prev => ({
      ...prev,
      [telefono]: { ...prev[telefono], sinLeer: 0 }
    }));
  };

  const totalUnread = Object.values(conversations).reduce((a, c) => a + (c.sinLeer || 0), 0);
  const totalVentas = clients.filter(c => c.status === "Cerrado").reduce((a, c) => a + c.totalGastado, 0);
  ...
  const convList = Object.entries(conversations).map(([tel, conv]) => {
    const client = clients.find(c => c.id === tel);
    return { telefono: tel, nombre: client?.nombre || tel, ...conv };
  });
    ...
    <div style={{ display: "flex", height: "100vh", background: "#060c18", fontFamily: "'DM Sans', sans-serif", color: "#e2e8f0", overflow: "hidden" }}>
      ...
      {notification && (
        <div className="notif">
          ...
          <div style={{ fontSize:13, fontWeight:600, color:"#e2e8f0" }}>{notification.nombre}</div>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
        ...
        <div style={{ padding:"14px 24px", borderBottom:"1px solid #0a1628", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <button className="btn-g" style={{ fontSize:11 }} onClick={simularMensaje}>🧪 Simular mensaje</button>
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
              <div style={{ width:7, height:7, borderRadius:"50%", background:"#10b981", animation:"pulse 2s infinite" }} />
              <span style={{ fontSize:11, color:"#10b981", fontWeight:600 }}>N8N conectado</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ flex:1, overflow:"auto", padding: section==="conversations"?0:"22px" }} className="anim">
          ...
          {section==="dashboard" && (
            <div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                    ...
                    return (
                      <div key={status} style={{ marginBottom:12 }}>
                        ...
                        <div style={{ height:3, background:"#07111f", borderRadius:4, overflow:"hidden" }}>
                          <div style={{ height:"100%", width:`${pct}%`, background:s.dot, borderRadius:4, transition:"width 0.5s" }} />
                        </div>
                      </div>
                    );
                  })}
                ...
                <div className="card" style={{ padding:20 }}>
                  <h3 style={{ fontSize:12, fontWeight:700, marginBottom:16, color:"#64748b", textTransform:"uppercase", letterSpacing:1 }}>Actividad WhatsApp hoy</h3>
                  {[
                    { label:"Mensajes recibidos", value:stats.mensajeshoy, color:"#3b82f6" },
                    { label:"Atendidos por bot", value:stats.bot, color:"#10b981" },
                    { label:"Transferidos a ti", value:stats.humano, color:"#f59e0b" },
                    { label:"Ventas por bot", value:stats.ventas, color:"#8b5cf6" },
                  ].map((s,i) => (
                    <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 0", borderBottom:i<3?"1px solid #0a1628":"none" }}>
                      <span style={{ fontSize:13, color:"#475569" }}>{s.label}</span>
                      <span style={{ fontSize:18, fontWeight:700, fontFamily:"'Syne'", color:s.color }}>{s.value}</span>
                    </div>
                  ))}
                  <div style={{ marginTop:14, padding:"10px 14px", background:"#07111f", borderRadius:8, display:"flex", alignItems:"center", gap:8 }}>
                    <div style={{ width:6, height:6, borderRadius:"50%", background:"#10b981", animation:"pulse 2s infinite" }} />
                    <span style={{ fontSize:12, color:"#10b981" }}>Bot activo — respondiendo automáticamente</span>
                  </div>
                </div>
                ...
                <div className="card" style={{ padding:20 }}>
                  ...
                  {convList.length === 0 && <div style={{ fontSize:13, color:"#334155", textAlign:"center", padding:"20px 0" }}>Sin conversaciones aún — usa "Simular mensaje" para probar</div>}
                  {convList.slice(0,4).map(conv => (
                    <div key={conv.telefono} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14, cursor:"pointer" }} onClick={() => { setSection("conversations"); setSelectedPhone(conv.telefono); }}>
          ...
          {section==="conversations" && (
            <div style={{ display:"flex", height:"100%" }}>
              <div style={{ width:280, borderRight:"1px solid #0a1628", overflowY:"auto", flexShrink:0 }}>
                ...
                {convList.length === 0 && (
                  <div style={{ padding:24, textAlign:"center", color:"#334155", fontSize:13 }}>
                    ...
                    <br/><br/>
                    <button className="btn-p" style={{ fontSize:11 }} onClick={simularMensaje}>Simular mensaje</button>
                  </div>
                )}
                {convList.map(conv => (
              ...
              {selectedPhone ? (() => {
                ...
                const client = clients.find(c=>c.id===selectedPhone);
                return (
                  <div style={{ flex:1, display:"flex", flexDirection:"column" }}>
                    <div style={{ padding:"12px 20px", borderBottom:"1px solid #0a1628", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        <div style={{ width:36, height:36, borderRadius:"50%", background:"linear-gradient(135deg,#1e3a5f,#1d4ed8)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, fontWeight:700 }}>{client?.nombre?.[0]||"?"}</div>
                        <div>
                          <div style={{ fontSize:14, fontWeight:600 }}>{client?.nombre||selectedPhone}</div>
                          <div style={{ fontSize:11, color:"#334155" }}>{client?.telefono}</div>
                        </div>
                      </div>
                      <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                        ...
                        <select className="input" value={client?.status||"Nuevo"} style={{ width:130, fontSize:12 }}
                          onChange={e => setClients(prev => prev.map(c => c.id===selectedPhone ? {...c,status:e.target.value} : c))}>
                    ...
                    <div style={{ flex:1, overflowY:"auto", padding:20, display:"flex", flexDirection:"column", gap:10 }}>
                      {conv.mensajes.length === 0 && (
                        <div style={{ textAlign:"center", color:"#334155", fontSize:13, marginTop:40 }}>Sin mensajes aún</div>
                      )}
                  ...
                  <div key={client.id} className="card" style={{ padding:"14px 20px", display:"flex", alignItems:"center", gap:14, marginBottom:10 }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                      ...
                      <span style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"3px 10px", borderRadius:20, fontSize:11, fontWeight:600, background:s.bg, color:s.text }}>
                        <span style={{ width:6, height:6, borderRadius:"50%", background:s.dot, display:"inline-block" }} />{client.status}
                      </span>
                      <select className="input" value={client.status} style={{ width:130, fontSize:12 }}
                        onChange={e => setClients(prev => prev.map(c => c.id===client.id ? {...c,status:e.target.value} : c))}>
                        {STATUSES.map(s => <option key={s}>{s}</option>)}
                      </select>
                      <button className="btn-g" style={{ fontSize:11 }} onClick={() => { setSection("conversations"); setSelectedPhone(client.id); }}>Ver chat</button>
                    </div>
                  </div>
                );
              })}
                  ...
                  <div className="modal" onClick={e=>e.stopPropagation()}>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                      ...
                      <div style={{ display:"flex", gap:8, marginTop:4 }}>
                        ...
                        <button className="btn-p" onClick={()=>{ if(newClient.nombre){ setClients([...clients,{...newClient,id:newClient.telefono||Date.now().toString(),totalGastado:0}]); setShowAddClient(false); setNewClient({nombre:"",telefono:"",status:"Nuevo",notas:""}); }}} style={{ flex:1 }}>Agregar</button>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {section==="combos" && (
            <div>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:18 }}>
                <p style={{ fontSize:13, color:"#475569" }}>{combos.filter(c=>c.activo).length} combos activos</p>
                ...
                {combos.map(combo => (
                  <div key={combo.id} className="card" style={{ padding:20, opacity:combo.activo?1:0.5 }}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:12 }}>
                      ...
                      <span style={{ fontSize:10, fontWeight:700, padding:"3px 8px", borderRadius:4, background:combo.activo?"#0d3320":"#1a1a2e", color:combo.activo?"#10b981":"#475569" }}>{combo.activo?"ACTIVO":"INACTIVO"}</span>
                    </div>
                    ...
                    <button className="btn-g" style={{ width:"100%", marginTop:12, fontSize:12 }}
                      onClick={()=>setCombos(combos.map(c=>c.id===combo.id?{...c,activo:!c.activo}:c))}>
                ...
                <div className="modal-bg" onClick={()=>setShowAddCombo(false)}>
                  <div className="modal" onClick={e=>e.stopPropagation()}>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                      ...
                      <input className="input" placeholder="Stock" type="number" value={newCombo.stock} onChange={e=>setNewCombo({...newCombo,stock:e.target.value})} />
                      <div style={{ display:"flex", gap:8, marginTop:4 }}>
                        <button className="btn-g" onClick={()=>setShowAddCombo(false)} style={{ flex:1 }}>Cancelar</button>
                        <button className="btn-p" onClick={()=>{ if(newCombo.nombre){ setCombos([...combos,{...newCombo,id:Date.now(),precio:parseFloat(newCombo.precio)||0,stock:parseInt(newCombo.stock)||0,activo:true}]); setShowAddCombo(false); setNewCombo({nombre:"",descripcion:"",precio:"",stock:""}); }}} style={{ flex:1 }}>Agregar</button>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {section==="pipeline" && (
            <div>
              <p style={{ fontSize:13, color:"#475569", marginBottom:18 }}>Vista kanban de clientes por etapa de venta</p>
                ...
                {STATUSES.map(status => {
                  ...
                  return (
                    <div key={status} style={{ minWidth:160, flex:1 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10, padding:"7px 12px", background:s.bg, borderRadius:8 }}>
                        <span style={{ width:7, height:7, borderRadius:"50%", background:s.dot, display:"inline-block" }} />
                        <span style={{ fontSize:12, fontWeight:700, color:s.text }}>{status}</span>
                        <span style={{ marginLeft:"auto", fontSize:11, color:s.text, opacity:0.7 }}>{statusClients.length}</span>
                      </div>
                      {statusClients.map(client => (
                        <div key={client.id} style={{ background:"#0a1628", border:"1px solid #0f2040", borderRadius:10, padding:12, marginBottom:8, cursor:"pointer", transition:"all 0.2s" }}
                          onClick={()=>{ setSection("conversations"); setSelectedPhone(client.id); }}>
                          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
                            <div style={{ width:26, height:26, borderRadius:"50%", background:"linear-gradient(135deg,#1e3a5f,#1d4ed8)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700 }}>{client.nombre[0]}</div>
                      ...
                      {statusClients.length===0 && <div style={{ padding:"18px 12px", textAlign:"center", color:"#1e3a5f", fontSize:12 }}>Vacío</div>}
                    </div>
                  );
                })}
              </div>
