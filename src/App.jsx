...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nav-item.active {
    ...
    color: var(--accent); border-color: rgba(244,114,182,.2);
    font-weight: 600;
  }
  ...
  .card:hover { border-color: rgba(244,114,182,.3); box-shadow: 0 4px 20px rgba(0,0,0,.2); }

  .btn {
    display: inline-flex; align-items: center; gap: 6px;
    padding: 8px 16px; border-radius: 9px; border: none;
    cursor: pointer; font-size: 12px; font-weight: 600;
    font-family: Poppins, sans-serif; transition: all .18s ease;
    white-space: nowrap;
  }
  .btn-primary {
    background: linear-gradient(135deg, #f472b6, #a78bfa);
    color: white; box-shadow: 0 2px 12px rgba(244,114,182,.25);
  }
  .btn-primary:hover { opacity:.9; transform:translateY(-1px); box-shadow:0 4px 18px rgba(244,114,182,.35); }
  ...
  .btn-icon { padding: 7px; border-radius: 8px; aspect-ratio: 1; justify-content: center; }

  .input {
    background: var(--bg3); border: 1.5px solid var(--border);
    border-radius: 9px; padding: 9px 13px; color: var(--text);
    font-size: 13px; font-family: Poppins, sans-serif;
    width: 100%; transition: all .18s ease; outline: none;
  }
  ...
  .conv-item.active::before {
    ...
    width: 3px; background: linear-gradient(180deg, #f472b6, #a78bfa);
  ...
  .modal {
    background: var(--bg2); border: 1px solid var(--border);
    border-radius: 18px; padding: 26px; width: 100%;
    max-width: 420px; box-shadow: 0 20px 60px rgba(0,0,0,.5);
  ...
  .stat-card:hover { transform:translateY(-2px); box-shadow:0 6px 24px rgba(0,0,0,.2); border-color: rgba(244,114,182,.2); }
...
export default function App() {
  ...
  // ── Auto-scroll: al cambiar de chat, al cargar, cuando llegan mensajes nuevos ──
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const prev = prevConvRef.current[selectedPhone]?.mensajes || [];
    const curr = conversations[selectedPhone]?.mensajes || [];
    if (curr.length !== prev.length || isFirstLoad.current) {
  ...
  const updateClientStatus = async (id, status) =>
    updateDoc(doc(db, "clientes", id), { status });
  ...
  const enviarMensaje = async () => {
    ...
    await setDoc(doc(db, "conversaciones", selectedPhone),
      { ...conv, mensajes, ultimoMsg: newMsg, ultimoTiempo: tiempo }, { merge: true });
    try {
  ...
  const theme = {
    "--bg":      dark ? "#0d0f18" : "#f4f6fb",
    "--bg2":     dark ? "#13161f" : "#ffffff",
    "--bg3":     dark ? "#1c2033" : "#f0f2f8",
    "--border":  dark ? "#232740" : "#e2e6f0",
    "--text":    dark ? "#eef0f8" : "#0d0f18",
    "--text2":   dark ? "#8892b0" : "#64748b",
    "--text3":   dark ? "#3d4565" : "#94a3b8",
    "--accent":  "#f472b6",
    "--accent2": "#a78bfa",
    "--accent3": "#34d399",
  };
  ...
  if (loading) return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center",
      height:"100vh", background:"#0d0f18", flexDirection:"column", gap:14 }}>
      <style>{GLOBAL_CSS}</style>
      <div style={{ width:44, height:44, border:"3px solid rgba(244,114,182,.2)",
        borderTop:"3px solid #f472b6", borderRadius:"50%",
        animation:"spin 1s linear infinite" }} />
      <div style={{ color:"#f472b6", fontFamily:"Poppins,sans-serif",
        fontSize:13, fontWeight:600, letterSpacing:".5px" }}>Cargando DND Boutique...</div>
    </div>
  );

  const sharedProps = {
    clients, combos, conversations, convList, selectedPhone, setSelectedPhone,
    newMsg, setNewMsg, enviarMensaje, toggleBot, chatEndRef, totalVentas,
    conversion, totalUnread, showAddClient, setShowAddClient, showAddCombo,
    setShowAddCombo, newClient, setNewClient, newCombo, setNewCombo,
    addCombo, addClient, updateClientStatus, setCombos, db, dark,
    scrollToBottom,
  };

  return (
    <div style={{ ...theme, display:"flex", height:"100vh", background:"var(--bg)",
      fontFamily:"'Poppins',sans-serif", color:"var(--text)", overflow:"hidden" }}>
      <style>{GLOBAL_CSS}</style>
        ...
        <div className="modal-overlay" onClick={() => setShowNewChat(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize:12, color:"var(--text3)", marginBottom:20 }}>
              Inicia un chat enviando el primer mensaje
            </p>
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              <input className="input" placeholder="Número WhatsApp (ej: 50372345678)"
                value={newChatPhone} onChange={e => setNewChatPhone(e.target.value)} />
              <textarea className="input" placeholder="Mensaje inicial..." rows={3}
                style={{ resize:"none" }} value={newChatMsg}
                onChange={e => setNewChatMsg(e.target.value)} />
              <div style={{ display:"flex", gap:10, marginTop:4 }}>
                <button className="btn btn-ghost" style={{ flex:1 }}
                  onClick={() => setShowNewChat(false)}>Cancelar</button>
                <button className="btn btn-primary" style={{ flex:1 }}
                  onClick={iniciarChat}>Enviar →</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <aside className="desktop-sidebar" style={{
        ...
        <div style={{ padding:"6px 10px 22px" }}>
          ...
          <div style={{ fontSize:10, color:"var(--text3)", marginTop:1,
            letterSpacing:".5px", fontWeight:500 }}>CRM · WhatsApp IA</div>
        </div>
        ...
        {/* Nuevo chat */}
        <button className="btn btn-primary" style={{ width:"100%", marginBottom:12, justifyContent:"center" }}
        ...
        <div style={{ borderTop:"1px solid var(--border)", paddingTop:14,
          ...
          <button onClick={() => setDark(!dark)} style={{
            ...
            borderRadius:20, padding:"3px 12px", cursor:"pointer", fontSize:14,
          }}>{dark ? "☀️" : "🌙"}</button>
        </div>
      </aside>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
        ...
        <header style={{
          ...
          <div>
            <h1 style={{ fontSize:17, fontWeight:700, letterSpacing:"-.2px" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            {/* Mobile toggle */}
            <button className="mobile-only" onClick={() => setDark(!dark)} style={{
              background:"var(--bg3)", border:"1px solid var(--border)",
            ...
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
              ...
              <span style={{ fontSize:11, color:"var(--accent3)", fontWeight:500 }}>
                Firebase + N8N activos
              </span>
            </div>
          </div>
        </header>
        ...
        <main style={{
          ...
          padding: section === "conversations" ? 0 : "24px",
          paddingBottom: section === "conversations" ? 0 : "24px",
        }} className="fade-up">
          <MainContent section={section} {...sharedProps} mobile={false} />
        </main>
      </div>
      ...
      <div className="mobile-tabbar">
        {NAV.map(item => (
          <div key={item.id}
            ...
            onClick={() => { setSection(item.id); setSelectedPhone(null); }}>
...
function MainContent(props) {
  ...
  if (section === "conversations") return <Conversations {...props} />;
  if (section === "clients")       return <Clients {...props} />;
  if (section === "combos")        return <Combos {...props} />;
  if (section === "pipeline")      return <Pipeline {...props} />;
  return null;
}

// ─── DASHBOARD ───────────────────────────────────────────────────
function Dashboard({ clients, combos, totalVentas, conversion, totalUnread, dark }) {
  const STATUSES = ["Nuevo", "Interesado", "Cotizado", "Cerrado", "Perdido"];
  return (
    <div style={{ maxWidth:900 }}>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:12, marginBottom:20 }}>
        {[
          ...
          { label:"Sin leer",     value: totalUnread,                                                 icon:"◎", color:"var(--accent)" },
        ].map((s, i) => (
          <div key={i} className="stat-card fade-up" style={{ animationDelay:`${i*0.05}s` }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
              <span style={{ fontSize:10, color:"var(--text3)", fontWeight:600,
                textTransform:"uppercase", letterSpacing:"1px" }}>{s.label}</span>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
        <div className="card" style={{ padding:20 }}>
          <h3 style={{ fontSize:12, fontWeight:700, marginBottom:16, color:"var(--text3)",
            textTransform:"uppercase", letterSpacing:"1px" }}>Pipeline</h3>
          {STATUSES.map(status => {
        ...
        <div className="card" style={{ padding:20 }}>
          <h3 style={{ fontSize:12, fontWeight:700, marginBottom:16, color:"var(--text3)",
            textTransform:"uppercase", letterSpacing:"1px" }}>Combos activos</h3>
          {combos.filter(c => c.activo).map(combo => (
            <div key={combo.id} style={{ display:"flex", justifyContent:"space-between",
              alignItems:"center", padding:"10px 14px", background:"var(--bg3)",
              borderRadius:10, marginBottom:8 }}>
              <div>
                <div style={{ fontSize:13, fontWeight:600 }}>◇ {combo.nombre}</div>
                <div style={{ fontSize:11, color:"var(--text3)", marginTop:1 }}>{combo.descripcion}</div>
              </div>
...
function Conversations({
  conversations, convList, selectedPhone, setSelectedPhone,
  newMsg, setNewMsg, enviarMensaje, toggleBot, chatEndRef,
  db, dark, scrollToBottom, mobile,
}) {
  ...
  const ChatList = (
    <div style={{ display:"flex", flexDirection:"column", height:"100%",
      ...
      <div style={{ flex:1, overflowY:"auto" }}>
        ...
        {convList.map(c => (
          <div key={c.telefono}
            className={`conv-item ${selectedPhone === c.telefono ? "active" : ""}`}
            onClick={() => setSelectedPhone(c.telefono)}>
            <div style={{ display:"flex", gap:11, alignItems:"center" }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <span style={{ fontSize:12, color:"var(--text3)", overflow:"hidden",
                  ...
                  <div style={{ display:"flex", gap:4, alignItems:"center", flexShrink:0 }}>
                    {c.botActivo && (
                      <span style={{ fontSize:9, color:"var(--accent)",
                        background:"rgba(244,114,182,.1)", padding:"1px 6px",
                        borderRadius:4, fontWeight:700 }}>BOT</span>
                    )}
                    {c.sinLeer > 0 && (
    ...
    <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:11 }}>
          {mobile && (
            <button className="btn btn-ghost btn-sm btn-icon"
              onClick={() => setSelectedPhone(null)} style={{ marginRight:4 }}>
              ←
            </button>
          )}
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"20px",
        ...
        {(conv.mensajes || []).length === 0 && (
          <div style={{ textAlign:"center", color:"var(--text3)", marginTop:60, fontSize:13 }}>
            Sin mensajes aún
          </div>
        )}
            ...
            <div className="bubble" style={{
              ...
              {msg.from === "bot" && (
                <div style={{ fontSize:10, color:"var(--accent2)",
                  fontWeight:700, marginBottom:4, letterSpacing:".3px" }}>
                  🤖 BOT · N8N
                </div>
              )}
              {msg.texto}
              <div style={{ fontSize:10, marginTop:5, textAlign:"right",
                color: msg.from === "user" ? "rgba(255,255,255,.6)" : "var(--text3)" }}>
                {msg.tiempo}
              </div>
            </div>
          </div>
      ...
      <div style={{ padding:"12px 20px", borderTop:"1px solid var(--border)",
        display:"flex", gap:10, background:"var(--bg2)", flexShrink:0 }}>
        <input className="input" value={newMsg}
          onChange={e => setNewMsg(e.target.value)}
          onKeyDown={handleKeyDown}
          placeholder="Escribe un mensaje... (Enter para enviar)"
          style={{ flex:1 }} />
        <button className="btn btn-primary" onClick={enviarMensaje}
          style={{ paddingLeft:20, paddingRight:20 }}>→</button>
      </div>
    </div>
  ) : (
    <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center",
      flexDirection:"column", gap:10, color:"var(--text3)" }}>
  ...
  if (mobile) {
    if (selectedPhone) return ChatArea;
    return <div style={{ height:"100%" }}>{ChatList}</div>;
  }
    ...
    <div style={{ maxWidth:860 }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
        {filtered.map((client, i) => {
          ...
          return (
            <div key={client.id} className="card fade-up"
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
                {client.totalGastado > 0 && (
                  <span style={{ fontSize:14, fontWeight:800, color:"var(--accent3)" }}>
                    ${client.totalGastado}
                  </span>
                )}
                <span className="tag" style={{ background:m.bg, color:m.text }}>
                ...
                <select className="input" value={client.status || "Nuevo"}
        ...
        <div className="modal-overlay" onClick={() => setShowAddClient(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              ...
              <input className="input" placeholder="Teléfono (+503...)"
                value={newClient.telefono} onChange={e => setNewClient({...newClient, telefono:e.target.value})} />
              <select className="input" value={newClient.status}
                onChange={e => setNewClient({...newClient, status:e.target.value})}>
                {STATUSES.map(s => <option key={s}>{s}</option>)}
              </select>
              <textarea className="input" placeholder="Notas..." rows={3}
                style={{ resize:"none" }} value={newClient.notas}
                onChange={e => setNewClient({...newClient, notas:e.target.value})} />
              <div style={{ display:"flex", gap:10, marginTop:4 }}>
                <button className="btn btn-ghost" style={{ flex:1 }}
                  onClick={() => setShowAddClient(false)}>Cancelar</button>
                <button className="btn btn-primary" style={{ flex:1 }}
...
// ─── COMBOS ──────────────────────────────────────────────────────
function Combos({ combos, showAddCombo, setShowAddCombo, newCombo, setNewCombo, addCombo, db }) {
  return (
    <div style={{ maxWidth:900 }}>
      <div style={{ display:"flex", justifyContent:"space-between",
        marginBottom:18, flexWrap:"wrap", gap:10, alignItems:"center" }}>
        <p style={{ fontSize:12, color:"var(--text3)" }}>
          {combos.filter(c => c.activo).length} activos · {combos.length} total
        </p>
        <button className="btn btn-primary" onClick={() => setShowAddCombo(true)}>
          + Nuevo combo
        </button>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))", gap:14 }}>
        {combos.map((combo, i) => (
          <div key={combo.id} className="card fade-up"
            style={{ padding:20, opacity:combo.activo ? 1 : 0.55,
            ...
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:14 }}>
              ...
              <button className="btn btn-danger btn-icon btn-sm"
                onClick={() => deleteDoc(doc(db, "combos", combo.id))}>✕</button>
            </div>
            <h3 style={{ fontSize:15, fontWeight:700, marginBottom:4 }}>◇ {combo.nombre}</h3>
            <p style={{ fontSize:12, color:"var(--text3)", marginBottom:16,
              lineHeight:1.5 }}>{combo.descripcion}</p>
            <div style={{ display:"flex", justifyContent:"space-between",
              alignItems:"center", marginBottom:14 }}>
              <span style={{ fontSize:22, fontWeight:800, color:"var(--accent3)" }}>
                ${combo.precio}
              </span>
      ...
      {showAddCombo && (
        <div className="modal-overlay" onClick={() => setShowAddCombo(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              <input className="input" placeholder="Nombre del combo"
                value={newCombo.nombre} onChange={e => setNewCombo({...newCombo, nombre:e.target.value})} />
              <textarea className="input" placeholder="Descripción" rows={3}
                style={{ resize:"none" }}
                value={newCombo.descripcion} onChange={e => setNewCombo({...newCombo, descripcion:e.target.value})} />
              <input className="input" placeholder="Precio ($)" type="number"
                value={newCombo.precio} onChange={e => setNewCombo({...newCombo, precio:e.target.value})} />
              <input className="input" placeholder="Stock disponible" type="number"
                value={newCombo.stock} onChange={e => setNewCombo({...newCombo, stock:e.target.value})} />
              <div style={{ display:"flex", gap:10, marginTop:4 }}>
                <button className="btn btn-ghost" style={{ flex:1 }}
                  onClick={() => setShowAddCombo(false)}>Cancelar</button>
                <button className="btn btn-primary" style={{ flex:1 }}
                  onClick={addCombo}>Agregar</button>
              </div>
            </div>
          </div>
        </div>
