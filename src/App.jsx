...
export default function App() {
  ...
  return (
    <div style={{ ...theme, display: "flex", height: "100vh", background: "var(--bg)", fontFamily: "inherit", color: "var(--text-main)", overflow: "hidden" }}>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                ...
                <button className="btn btn-primary" style={{ flex: 1, justify: "center" }} onClick={iniciarChat}>Enviar →</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                ...
                <button className="btn btn-primary" style={{ flex: 1, justify: "center" }} onClick={guardarEditChat}>Guardar</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                ...
                <button className="btn btn-primary" style={{ flex: 1, justify: "center" }} onClick={saveEditCombo}>Guardar</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Sidebar Escritorio - Inspirado en tu captura */}
      <aside className="desktop-sidebar">
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 16, display: "flex", alignItems: "center", justify: "space-between" }}>
          <span style={{ fontSize: 11, color: "#94a3b8" }}>{dark ? "Modo oscuro" : "Modo claro"}</span>
          <button onClick={() => setDark(!dark)} style={{ background: "rgba(255,255,255,0.08)", border: "none", borderRadius: 20, padding: "4px 12px", cursor: "pointer", color: "#ffffff", fontSize: 13 }}>{dark ? "☀️" : "🌙"}</button>
        </div>
      </aside>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <header style={{ padding: "16px 24px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", background: "var(--bg-card)", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button className="mobile-only" onClick={() => setDark(!dark)} style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 20, padding: "4px 10px", cursor: "pointer", color: "var(--text-main)", fontSize: 13 }}>{dark ? "☀️" : "🌙"}</button>
        ...
        {NAV.map(item => (
          <div key={item.id} className={`mobile-tab ${section === item.id ? "active" : ""}`} onClick={() => { setSection(item.id); setSelectedPhone(null); }}>
            <span className="mobile-tab-icon" style={{ position: "relative" }}>
              {item.icon}
              {item.id === "conversations" && chatsUnread > 0 && (
                <span style={{ position: "absolute", top: -4, right: -4, background: "var(--accent)", color: "white", fontSize: 8, fontWeight: 700, borderRadius: "50%", width: 14, height: 14, display: "flex", alignItems: "center", justify: "center" }}>{chatsUnread}</span>
              )}
            </span>
        ...
        <div className="card" style={{ padding: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {combos.filter(c => c.activo).map(combo => (
              <div key={combo.id} style={{ display: "flex", justify: "space-between", align: "center", padding: "12px", background: "var(--bg)", borderRadius: 12 }}>
                <div>
...
function Conversations({ conversations, convList, selectedPhone, setSelectedPhone, newMsg, setNewMsg, enviarMensaje, toggleBot, chatEndRef, clients, updateClientStatus, searchConv, setSearchConv, abrirEditChat }) {
  ...
  const ChatList = (
    <div className={`chat-list-container ${listHiddenClass}`} style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--bg-card)", borderRight: "1px solid var(--border)", width: 300, flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        ...
        {convList.map(c => (
          <div key={c.telefono} className={`conv-item ${selectedPhone === c.telefono ? "active" : ""}`} onClick={() => setSelectedPhone(c.telefono)}>
            <div style={{ display: "flex", gap: 12, align: "center" }}>
              <div style={{ width: 36, height: 36, borderRadius: "50%", background: "var(--accent)", display: "flex", align: "center", justify: "center", fontSize: 13, fontWeight: 700, color: "white", flexShrink: 0 }}>
                {c.nombre[0]?.toUpperCase()}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", justify: "space-between", align: "center", marginBottom: 4 }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: "var(--text-main)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.nombre}</span>
                  <span style={{ fontSize: 9, color: "var(--text-light)" }}>{c.ultimoTiempo}</span>
                </div>
                <div style={{ display: "flex", justify: "space-between", align: "center" }}>
                  <span style={{ fontSize: 11, color: "var(--text-light)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "80%" }}>{c.ultimoMsg}</span>
                  <div style={{ display: "flex", gap: 4, align: "center", flexShrink: 0 }}>
                    ...
                    {c.sinLeer > 0 && <span style={{ background: "#ef4444", color: "white", fontSize: 8, fontWeight: 700, borderRadius: "50%", width: 14, height: 14, display: "flex", align: "center", justify: "center" }}>!</span>}
                  </div>
                </div>
              </div>
            </div>
  ...
  const ChatArea = conv ? (
    <div className={`chat-area-container ${areaHiddenClass}`} style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, height: "100%" }}>
      <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border)", display: "flex", align: "center", justify: "space-between", background: "var(--bg-card)", flexShrink: 0 }}>
        <div style={{ display: "flex", align: "center", gap: 10 }}>
          ...
          <div style={{ width: 34, height: 34, borderRadius: "50%", background: "var(--accent)", display: "flex", align: "center", justify: "center", fontWeight: 700, color: "white", fontSize: 13 }}>
        ...
        <div style={{ display: "flex", gap: 8, align: "center" }}>
          {clienteRelacionado && (
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: 10, background: "var(--bg)" }}>
        ...
        {(conv.mensajes || []).map((msg, i) => (
          <div key={i} style={{ display: "flex", justify: msg.from === "client" ? "flex-start" : "flex-end" }}>
    ...
    <div className="chat-area-container chat-area-hidden" style={{ flex: 1, display: "flex", align: "center", justify: "center", flexDirection: "column", gap: 8, color: "var(--text-light)" }}>
...
function Clients({ clients, showAddClient, setShowAddClient, newClient, setNewClient, addClient, updateClientStatus }) {
  ...
  return (
    <div style={{ maxWidth: 880, margin: "0 auto", width: "100%" }}>
      <div style={{ display: "flex", justify: "space-between", marginBottom: 20, gap: 12, flexWrap: "wrap" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {filtered.map((client, i) => {
          ...
          return (
            <div key={client.id} className="card fade-up" style={{ padding: "14px", display: "flex", align: "center", gap: 14, flexWrap: "wrap", animationDelay: `${i * .02}s` }}>
              <div style={{ width: 34, height: 34, borderRadius: "50%", background: "var(--accent)", display: "flex", align: "center", justify: "center", fontSize: 13, fontWeight: 700, color: "white", flexShrink: 0 }}>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                ...
                <button className="btn btn-primary" style={{ flex: 1, justify: "center" }} onClick={addClient}>Agregar</button>
              </div>
            </div>
          </div>
        </div>
...
function Combos({ combos, showAddCombo, setShowAddCombo, showEditCombo, setShowEditCombo, editCombo, setEditCombo, saveEditCombo, newCombo, setNewCombo, addCombo, db }) {
  return (
    <div style={{ maxWidth: 920, margin: "0 auto", width: "100%" }}>
      <div style={{ display: "flex", justify: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 10, align: "center" }}>
        <p style={{ fontSize: 11, color: "var(--text-light)" }}>{combos.filter(c => c.activo).length} activos · {combos.length} total</p>
        <button className="btn btn-primary" onClick={() => setShowAddCombo(true)}>+ Nuevo combo</button>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 14 }}>
        {combos.map((combo, i) => (
          <div key={combo.id} className="card fade-up" style={{ padding: "18px", opacity: combo.activo ? 1 : .55, animationDelay: `${i * .04}s` }}>
            <div style={{ display: "flex", justify: "space-between", marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", justify: "space-between", align: "center", marginBottom: 14 }}>
            ...
            <button className="btn btn-ghost btn-sm" style={{ width: "100%", justify: "center" }}
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                ...
                <button className="btn btn-primary" style={{ flex: 1, justify: "center" }} onClick={addCombo}>Agregar</button>
              </div>
            </div>
          </div>
        </div>
        ...
        {STATUSES.map(status => {
          ...
          return (
            <div key={status} style={{ minWidth: 210, flex: "0 0 210px" }}>
              <div style={{ display: "flex", align: "center", gap: 8, marginBottom: 12, padding: "8px 12px", background: m.bg, borderRadius: 10 }}>
                ...
                {sc.map(client => (
                  <div key={client.id} className="card" style={{ padding: 12 }}>
                    <div style={{ display: "flex", align: "center", gap: 8, marginBottom: 6 }}>
                      <div style={{ width: 22, height: 22, borderRadius: "50%", background: "var(--accent)", display: "flex", align: "center", justify: "center", fontSize: 10, fontWeight: 700, color: "white", flexShrink: 0 }}>
                        {client.nombre[0]?.toUpperCase()}
                      </div>
                    ...
                    {client.notas && <div style={{ fontSize: 9, color: "var(--text-muted)", marginTop: 4, fontStyle: "italic" }}>📝 {client.notas.slice(0, 30)}</div>}
